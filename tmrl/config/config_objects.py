...
from tmrl.custom.custom_gym_interfaces import (
    TM2020Interface,
...
if cfg.PRAGMA_LIDAR:
    ...
    assert ALG_NAME == "SAC", f"{ALG_NAME} is not implemented here."
    # TRAIN_MODEL = DINOActorCritic
    # POLICY = DINOSquashedActor
...
if ALG_NAME == "SAC":
    AGENT = partial(
        ...
        betas_critic=ALG_CONFIG["BETAS_CRITIC"]
        if "BETAS_CRITIC" in ALG_CONFIG
        else None,
...
ENV_CLS = partial(
    GenericGymEnv, id=cfg.RTGYM_VERSION, gym_kwargs={"config": CONFIG_DICT}
)
...
DUMP_RUN_INSTANCE_FN = (
    None if cfg.PRAGMA_LIDAR else None
)  # dump_run_instance_images_dataset
LOAD_RUN_INSTANCE_FN = (
    None if cfg.PRAGMA_LIDAR else None
)  # load_run_instance_images_dataset
