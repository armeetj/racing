...
def update_run_instance(run_instance, training_cls):
    ...
    if ALG_NAME in ["SAC", "REDQSAC"]:
        ...
        if ALG_NAME == "SAC":
            if run_instance.agent.lr_actor != lr_actor:
                ...
                run_instance.agent.pi_optimizer = Adam(
        ...
        if (
            ...
            or run_instance.agent.alpha != alpha
        ):
            run_instance.agent.lr_entropy = lr_entropy
            run_instance.agent.alpha = alpha
            device = run_instance.device or (
            ...
            if run_instance.agent.learn_entropy_coef:
                ...
                run_instance.agent.alpha_t = torch.tensor(
                    float(run_instance.agent.alpha)
                ).to(device)
        ...
        if ALG_NAME == "REDQSAC":
            ...
            if (
                run_instance.agent.q_updates_per_policy_update
                != q_updates_per_policy_update
            ):
    ...
    max_training_steps_per_env_step = cfg.TMRL_CONFIG[
    ...
    if run_instance.update_model_interval != update_model_interval:
        ...
        logging.info(
            f"Model update interval changed to {update_model_interval} (old: {old})."
        )
    ...
    if run_instance.update_buffer_interval != update_buffer_interval:
        ...
        logging.info(
            f"Buffer update interval changed to {update_buffer_interval} (old: {old})."
        )
