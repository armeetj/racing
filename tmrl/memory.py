...
def check_samples_crc(
    ...
    print(
        f"DEBUG: CRC check passed. Time step: {debug_ts}, since reset: {debug_ts_res}"
    )
...
class Memory(ABC):
    ...
    def __getitem__(self, item):
        prev_obs, new_act, rew, new_obs, terminated, truncated, info = (
            self.get_transition(item)
        )
        if self.crc_debug:
