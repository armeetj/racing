...
def collate_torch(batch, device=None):
    ...
    if isinstance(elem, torch.Tensor):
        ...
        if (
            elem.numel() < 20000
        ):  # TODO: link to the relevant profiling that lead to this threshold
...
def partial_from_args(func: Union[str, callable], kwargs: Dict[str, str]):
    ...
    for key in keys:
        ...
        assert (
            key in params
        ), f"'{key}' is not a valid parameter of {func}. Valid parameters are {tuple(params.keys())}."
        param = params[key]
...
def git_info(path=None):
    ...
    status = get_output(
        "git status --short".split(), cwd=path
    )  # shows un-committed modified files
    ...
    if url.startswith("git@github.com:"):
        url = (
            "https://github.com/"
            + url[len("git@github.com:") : -len(".git")]
            + "/commit/"
            + rev
        )
...
def dump(obj, path):
    ...
    with DelayInterrupt():  # Continue to save even if SIGINT or SIGTERM is sent and raise KeyboardInterrupt afterwards.
        with open(tmp_path, "wb") as f:
            ...
            )  # dump temporary file (can fail)
        os.replace(
            tmp_path, path
        )  # replace with definitive name (this is supposedly atomic)
